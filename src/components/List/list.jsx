...
function List({ title, items, setAllLists, allLists, active, id }) {
	...
	return (
		<div>
			<div onClick={() => setAllLists(allLists.map((list) => ({...list, active: list.id === id && !list.active})))}
			className={`list ${active ? 'list_active' : ''}`}>
				<h2 className='list__title'>{title}</h2>
				<span className='list__itemsCount'>{items.length}</span>
			</div>

			{active && <div className="list__itemAdding"> 
				<span className="list__itemAdding-button">+ List item</span>

				<form 
				onSubmit={(e) => {
					e.preventDefault();
					setAllLists(allLists.map((list) => ({...list, items: [newItem, ...list.items]})))
				}} 
